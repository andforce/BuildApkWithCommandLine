...
aapt2 link --proto-format -o tmp.apk -I ~/android-sdk/platforms/android-28/android.jar -I /home/dy/Desktop/aapt/smartisanos.jar res.zip --manifest app/src/main/AndroidManifest.xml --auto-add-overlay --java gen

unzip tmp.apk -d staging

mkdir -p staging/{manifest,dex} && mv staging/AndroidManifest.xml staging/manifest

javac -source 1.8 -target 1.8 -bootclasspath $JAVA_HOME/jre/lib/rt.jar -classpath ~/android-sdk/platforms/android-28/android.jar -d classes gen/com/andforce/buildapkwithcommandline/*.java app/src/main/java/com/andforce/buildapkwithcommandline/*.java

~/android-sdk/build-tools/28.0.3/dx --dex --output=staging/dex/classes.dex classes/

cd staging && zip -r ../base.zip * && cd -

java -jar bundletool-all-0.10.2.jar build-bundle --modules=base.zip --output=bundle.aab
